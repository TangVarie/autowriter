...
| 借来的"爆款"其实指标没验证过，而提示词里看不出来 | `render_flywheel_block` 渲染 `synthetic` 标记，整批有未验证卡时开头那句不再断言"真爆过" |
| 借阅失败和"这次没匹配"长得一模一样，用不上经验也没人知道 | `fetch_flywheel_lessons` 分 `borrowed` / `empty` / `not_configured` / `timeout` / `error` 五种结局，各自打点并带耗时 |
