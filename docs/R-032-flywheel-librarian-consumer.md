...
```jsonc
{
  ...
  // "draft_topic": <可选, 当前未填 —— 见 §6 待确认 2>
}
```

- 项目某列缺失 → 对应值为 `null`(契约约定馆员按缺失处理,不 500)。
- **未发送的字段**:`key_messages`(核心卖点)、`draft_topic`(选题)。见 §6。
...
autowriter 当前**只读取并注入这 5 个字段**(`memory.build_layered_system_prompt`
渲染进 P2):
...
> **当前未注入 prompt 的字段**:`tier`、`structure`、`transferable_tactic`、
> `source_note_id`。它们能正常收到、但不会进 system prompt。若 TV 侧希望
> `structure`(结构)/ `transferable_tactic`(可迁移手法)也被模型看到,告诉我,
> 我在 P2 那节补上即可(见 §6 待确认 3)。
...
1. **`key_messages`(核心卖点)要不要发?** 契约没列,我严格按契约没发。但它对匹配
   可能很有用 —— 要发就告诉我,一行加上。
2. **`draft_topic` 接什么数据源?** autowriter 当前 batch 没有独立"选题"字段,
   最接近的是 `key_messages`。要不要把 `key_messages` 映射进 `draft_topic`?
3. **响应里 `tier` / `structure` / `transferable_tactic` 要不要也注入 prompt?**
   当前只注入了 5 个字段(§4)。
4. **`tactics` 字段形态**:当前发的是 `project["tactics"]` 原始值(很可能是
   JSON 字符串)。TV 侧若 prompt-cache 这段,确认形态稳定即可。
