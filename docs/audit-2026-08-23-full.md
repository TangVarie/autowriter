...
| ROB-001/002 + SUP-011/012 · 抽 `generation_service.py` | 🚧 两步已做 | 编排已搬出 Streamlit + worker handler 已注册；**UI 还没切到入队**，SUP-012 去重未做 |
...
#### ROB-001/002 + SUP-011/012 —— 编排搬出 Streamlit，worker 接上（两步，未完）
...
2. **SUP-012（两份重复编排）没合。** 量过：`_queue_worker_impl`（457 行）与 `_quick_gen_worker`（373 行）按行匹配有 48% 重合，最长的共同片段 19 行，但中间穿插着不同的结构（一个循环 plan 并带 4 个缓存，一个只跑单个 plan）。合它要先逐处枚举差异——那两份**已经漂移过一次**（原 `app.py:3134` 的注释自陈）。这是本批剩下最大的一块。
