...
**另外发现一处、但刻意没修：`normalize()` 不去中文弯引号。** `_PUNCT_RE` 原来是两个字符串隐式拼接（后半截不是 raw 串），于是 `\\` 塌成一个反斜杠、把后面的 `|` 转义掉，而那两个 ASCII 双引号被当成串边界吃掉了。后果：`“` `”` 从来就不在字符类里。实测**同一篇稿子换个引号样式，Jaccard 掉到 0.333**，正好在 0.35 硬闸线之下，开头指纹也对不上——是一条能绕过查重的路子。

不在本次改的理由是具体的：`normalize` 是**存量指纹的计算口径**。改了它，新稿算出来的四字串就和库里几千条历史指纹对不上，查重在过渡期反而**更弱**，而 `backfill` 是按 `version_id` 幂等跳过的、不会重算已有行。要改必须配一次全量重算。所以这次只把字符串字面量改成合法的 raw 串（**编译结果逐字节相同**，有断言钉死），把缺口写成一条可执行的用例——修的时候它会红，逼人一起处理重算。
