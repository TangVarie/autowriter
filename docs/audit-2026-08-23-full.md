...
| 第 4 批 · 结构性 | ROB-001 · ROB-002 · SUP-011 · SUP-012 · SUP-024 · SUP-010 · COR-014 · COR-015 | 🚧 **八条都动过**(见 §0.5) —— SUP-024 / COR-015 / COR-014 / SUP-010 / SUP-012 ✅；ROB-001/002 + SUP-011 的 worker 侧就绪，**UI 切换待运维** |
...
| COR-014 · bottom-k 长短稿失真 | ✅ 已做 | 量化之后发现"改标准 bottom-k"**不够** —— 见下 |
| ROB-001/002 + SUP-011/012 · 抽 `generation_service.py` | 🚧 三步已做 | 编排已搬出 Streamlit + worker handler 已注册 + SUP-012 去重已做；**UI 还没切到入队**（等 worker 部署，用户拍板） |
| SUP-010 · DDL 挪出 `db.py` | ✅ 已做 | 关键不是「挪」，是挪完之后**终于跑得起来**了 —— 见下 |
...
#### ROB-001/002 + SUP-011 —— 编排搬出 Streamlit，worker 接上（UI 未切）

侦察结论比预想好得多：十四个编排函数里 **`st.` 属性访问是 0 次**（AST 数的，不是肉眼看的），只用 5 个模块级常量。真正把它们锁在 `app.py` 里的只有"定义写在那儿"这一件事。

**第一步的搬迁是逐字节的。** 21 个定义（14 个函数 + 5 个常量 + `_NullLock` 类）与搬走之前一个字符都没差，`tests/test_generation_service.py` 用 `git show` 取父提交的 `app.py` 逐个比对钉死。理由很实际：review 的人看着 1800 行 diff 分辨不出哪些是"搬"、哪些是"改"，而这一步的全部价值就是"可以断定行为没变"。`app.py` 5625 → 3831 行，UI 代码一行没动。

**第二步接上 worker**，三个判断值得记：

- **进度桥接用 dict 子类**。编排是拿一个 dict 当"进度总线"写的（Streamlit 那边把同一个对象放进 `session_state`，UI 直接轮询）。worker 里没有 session_state，所以用 `_JobStatus(dict)` 把 `progress`/`message`/`current` 的写入转成 `db.update_job_progress`。选子类而不是改编排签名，是因为编排里有几十处 `status[...] =`——换成回调要动的地方太多，而这一步的全部价值仍然是"行为没变"。
- **必须节流**。LLM 的 `progress_callback` 会几十上百次地写 `progress`，不节流的话一次批量生成能打出几千条 UPDATE，而 jobs 行上只有一个整数百分比有意义——那是拿数据库当日志用。
- **这两个 job 不幂等**。每个 plan 跑完就落库了，重试会把已经成功的那部分再生成一遍：用户看到凭空多出来的重复内容，而哪儿都不报错（job 那边看起来只是"重试了一次然后成功"）。所以 `db.insert_job` 对 `NON_IDEMPOTENT_JOB_KINDS` 强制 `max_attempts=1`，且 handler **不因为个别 plan 失败就抛异常**（抛出去只会触发整批重跑）。

**⚠️ 还没做完的两件，别把这条当已关闭：**

1. **Streamlit UI 仍走原来的 daemon thread 路径，没改成入队。** 也就是说 ROB-001（进程重启丢批次）/ ROB-002（JWT 中途过期）在 UI 那条路上**还没关掉**——这两步只是把 worker 这一侧准备好。切过来是**行为变更**（生成从"页面开着才跑"变成"关了页面也跑"），而且 runbook 记着 worker service **还没部署**——在那之前切过去等于生成整个不可用。这是个需要拍板的运维决策，不是我能顺手做的。
2. **SUP-012 已做**，但做法与审计原文不同——见下一节。

#### SUP-012 —— 差异列完之后，我改了做法

审计写的修法是"合并 `_queue_worker_impl` 与 `_quick_gen_worker`"。**先把差异逐条列完**（这是 COR-014 学到的那条：先量再改），然后改了做法。

**怎么列的**：写了一套替身 + **调用录音机**（`tests/genfakes.py`），判据不是"结果看起来对"，而是**逐次外部调用的序列**——调了谁、按什么顺序、关键入参是什么。

录出来的第一版结论是"主干 23 步逐步一致，差异 5 处"。**那是错的**：录音机只记了 `filter_soft_by_relevance` 的 `{n, has_context}`，没记那段**文本本身**；`generate_batch` 也只记了 count/engines，没记 `extra_instructions`。把值原样记下来之后又冒出两处。

> **教训（第三次应用同一条）：录音机记得不够细，得到的绿是假的。** 这和 §0.4 的"断言要钉住被禁止的形态"、COR-014 的"先量再改"是同一件事的不同侧面——**验证手段本身也要被验证**。

最终差异约 12 处，其中好几处是**有意的产品差异**而不是漂移：

| 差异 | 是漂移吗 |
|---|---|
| 队列**不支持图片**（plan 字典里根本没有 `image_prompt`/`images`，核过 UI） | ❌ 产品上就没做 |
| 队列的 `historical_titles` 并进跨批内存池，快速生成只看 DB | ❌ 队列的核心能力 |
| 进度消息 / 错误前缀带"计划 X/Y" | ❌ 两个面板本来就不同 |
| 跨 plan 的四层缓存 | ❌ 单批次不需要 |
| 取项目：逐个查 vs 批量预取 | ❌ 队列的 N+1 处理 |
| **空 `system_prompt` 时队列拒绝、快速生成照常跑** | ⚠️ **真分歧，见下** |

合成一个函数意味着六七个行为开关——**那不比两份清楚的实现好，只是把重复换成了配置复杂度**。所以改成抽掉真正重复的那两段：

- **`_call_generator`**：那组 ~18 个参数原来写了**四遍**（两条路径 × multi_role 两个分支）。加一个参数要改四处，漏一处不报错，只会让两条路径悄悄分叉——这正是 SUP-012 说的"下次还会漂"。
- **`_persist_and_check`**：落库 → 语义查重 → 硬约束校验。三步顺序不能动（查重放到硬约束后面的话，重生出来的新内容不会再过一遍硬约束），收成一处之后这个约束只需要守一遍。

`_queue_worker_impl` 457 → 412 行，`_quick_gen_worker` 373 → 328 行。

**唯一一处顺序变化**：`validator.filter_hard` 从落库之后挪到了之前（现在作为参数传进去）。它是纯列表推导（`validator.py:321`），语义完全一样。金序列里为这一处写了理由——**否则那份序列就退化成橡皮图章**：谁改红了就顺手改一下，它就不再证明任何事情。

**逐字节那条断言相应调整**：那两个编排从清单里去掉了（这次是有意改动）。去掉不是放松要求，是换了个更强的守卫——**录音能证明"动了但行为没变"，逐字节只能证明"没动过"**，而重构需要的是前者。另加一条断言防走捷径：那两个函数必须仍然被特征化测试覆盖着，不能靠"把名字从清单里删掉"变绿。

> ⚠️ **留给人拍板的一处**：项目 `system_prompt` 为空时，队列拒绝并报"项目未配置 System Prompt"，快速生成**照常生成**——拿一个空 base_prompt 去调模型，等于安静地产出一批不带项目人格的稿子。按本次审计的一贯口径应该统一到"拒绝"，但那是**用户可见的行为变更**（现在能跑的流程会开始报错），所以两边各自保留现状，没有偷偷改。

#### SUP-010 —— 关键不是「挪」，是挪完之后终于跑得起来了

`CREATE_TABLES_SQL` 是 `db.py` 里一个 1162 行的 Python 字符串。挪成 `migrations/000_baseline.sql` 让 `db.py` 从 4438 降到 3280 行，但那是副产品。**真正的问题是：没有任何代码执行过它，所以也没有任何东西验证过它。**

代价在这次审计里就撞见了：修 COR-014 时发现 `db.py` 那份 `deskcore_commit_fingerprints` 还带着"空开头的 title-only 稿会互相精确撞车"这个 bug——而 `migrations/001` 里同一个函数早就修好了。也就是说**任何一个新环境都会带着一个老 bug 出生**，而没人会发现。

所以这一条的实现重点是那个 harness（`tests/sql_parity_check.py` + 一个 CI 步骤），在一个空库上真的执行：

> 空库 → `000` → `001..005` → **再跑一遍全部**（验幂等）→ 抽查 7 张表 + 6 个函数是否都在 → 逐例比对下推 SQL 与 Python 算出来的数。

**这才是"消双写"真正的意思：不是只留一份，而是让两份必须对得上、对不上就报错。**

它上线当天就抓到两个真问题，两个都是我自己刚写进去的：

| # | 问题 | 后果 |
|---|---|---|
| 1 | `004` 用 `CREATE OR REPLACE` 建 `deskcore_check_drafts` | 基线已经是 005 那版（10 列），跑到 004 报 `cannot change return type` |
| 2 | `005` 里 6 参版的 `deskcore_commit_fingerprints` 用了裸 `CREATE FUNCTION` | 重跑报 `already exists`——**不幂等**，而 `migrations/README.md` 正好承诺了幂等 |

第 2 条尤其值得记：我在那个文件的注释里亲手写下"幂等: DROP + CREATE, 重复执行是干净 no-op"，而它其实不是。**写了一句关于代码的断言，和验证过那句断言，是两件事**——这正是 §0.4 那条教训的又一次复现。

顺带把 6 处指向 `db.CREATE_TABLES_SQL` / `db.py` 的 CI 断言改指基线文件。它们全是"两份 SQL 必须一致"的配对检查——挪走之后其中 3 处会静默地变成"拿 db.py 的 Python 代码去匹配 SQL 片段"，本地跑出来当场红了 3 个块。

#### COR-014 —— 量化之后，我给自己写的"修法"是错的

报告里这条的诊断是「bottom-k 的 Jaccard 在长短稿之间系统性偏低」，SUP-024 那步我还把它写成了一条 `xfail(strict=True)`，reason 里明写修法是「改成标准 bottom-k 估计」。**真去量之后，那个修法只对了一半，而我写的那条断言修完仍然是红的。**

先量。合成对照，短稿是长稿开头的**逐字**子串：

| 形状 | 真 Jaccard | 真包含度 | 现在算出来 | 改标准估计式之后 |
|---|---|---|---|---|
| 12 句 ⊂ 24 句 | 0.500 | 1.000 | 0.375 **reject** | 0.545 **reject** |
| 12 句 ⊂ 60 句 | 0.200 | 1.000 | 0.130 pass | 0.230 warn |
| 12 句 ⊂ 120 句 | 0.100 | 1.000 | 0.064 pass | 0.120 pass |
| 30 句 ⊂ 300 句 | 0.102 | 1.000 | 0.053 pass | 0.100 pass |
| 6 句 ⊂ 300 句 | 0.020 | 1.000 | 0.003 pass | 0.005 pass |

**7 例逐字抄袭，现在拦下 1 例；改成标准估计式之后，仍然是 1 例。**

两件事被我混成了一件：

- **估计确实有偏，而且方向固定。** 每篇各自取 bottom-k，两篇长度差得多时它们的第 k 小值差着量级，短稿 sketch 里那些落在长稿 sketch 之外的 hash 会被当成"长稿没有"，估计**系统性偏低**。偏差最大 0.125，而硬闸线就在 0.35。这一半的诊断是对的，改法也对：只在 `t = min(max(A), max(B))` 以下的 hash 区间上算——那个子域里两边的成员判定都是精确的，而 sha256 均匀，子域是全域的均匀随机样本。偏差降到最大 0.045，**存量指纹不用重算**。
- **但 Jaccard 这个指标在这个形状下结构上就不行。** 短稿整篇塞进长稿里，`J = |A|/|B|`——280 字抄进 2800 字的稿子里，**真值**就是 0.1。分母被长稿撑大，这是定义决定的，不是精度问题。再准的估计也够不着 0.35。

所以真正缺的是**另一个指标**：包含度 `|A∩B| / min(|A|,|B|)`——"短的那篇有多少比例出现在长的那篇里"，逐字抄袭时是 1.0，与长度差无关。加上它之后 **7/7 全部拦下**。

**阈值是从零分布定的，不是拍的。** 6000 次合成对照，按有效样本量 `m` 分桶（`m` = 受限子域里较小那边的元素个数，它才是决定噪声的量）：

| `m` 区间 | 无关两篇的包含度 p99 / max | 误杀（≥0.60） |
|---|---|---|
| 0–4 | 1.000 / 1.000 | 8 |
| 5–9 | 0.571 / 0.600 | 5 |
| 10–14 | 0.364 / 0.417 | 0 |
...
`m ≥ 15` 的 4707 次里无关稿子最高只到 0.333，离 0.60 有近一倍余量；真阳性全是 1.000。所以 `NGRAM_CONTAIN_HARD = 0.60`、`CONTAIN_MIN_SAMPLE = 15`——**低于下限就完全不发这一路的信号**。这一路刻意 fail-open（与本仓其它地方相反）：`m=4` 时无关稿子也能撞出 1.0，按硬闸处理就是**误杀**，而误杀比漏检更难被发现——用户只会觉得"这系统老让我重写"，然后没人再信这个闸。

**顺带把 sketch 从 200 提到 400。** 这不是"调大一点更好"，是判据问题：`m` 由长稿 sketch 的第 cap 小值决定，cap=200 时最常见那几个形状的 `m` 的 **p05 只有 12–14**，低于 15 的下限——也就是有 5% 以上的概率这一路直接哑火，抓不抓得到全看运气。cap=400 之后 p05 在 30 以上。只看中位数（cap=200 下也有 21）会得出"不用改"的错误结论。新旧 sketch 混着比是正确的（`t` 取两边最大值的较小者，精度退回旧的那一边，与今天持平），**所以仍然不需要重算存量指纹**。

**两道闸都要改，不只是查重那一道。** `deskcore_commit_fingerprints`（写入时在同一事务里重查，关掉 check→commit 的竞态窗口）用的是同一个有偏公式。`migrations/004` 的文件头自己写着"两者的 Jaccard 口径必须一致"。不一起改的话，同一对稿子会在两道闸上得到相反的结论，而短稿照搬长稿在写入侧完全没有防线。

**三处只有真跑才暴露的问题：**

1. **`WITH` 的作用域只有一条语句。** 把 Jaccard 和包含度拆成两条 `SELECT` 去读同一个 CTE，第二条报 `relation "metrics" does not exist`。
2. **两个 CTE 之间漏了逗号。**
3. **`commit` RPC 的签名变了。** 从 4 参变成 6 参，而只跑到 `004` 的库上还是 4 参——PostgREST 对"参数对不上"的报错文本里带 `does not exist`，**正好被 `rpc_missing()` 认成"函数压根不存在"**，于是整条原子写入路径静默退化成直插，竞态窗口重新打开，而日志只会说"migrations/001 还没跑?"（完全误导）。分两步升级的库上这一定会发生。现在的做法是先试 6 参、找不到再回退 4 参，两次都找不到才算真没跑迁移，并且回退时埋一行 telemetry。

前两个是我"读代码 + 肉眼 review"漏掉的——plpgsql 的函数体对 Python 来说只是个字符串，没有任何静态检查够得着它，而这些 SQL 是**查重硬闸的实现**。所以新增了 `tests/sql_parity_check.py` + 一个 CI 步骤：在真的 PostgreSQL 上跑一遍迁移，并**逐例比对 SQL 与 Python 算出来的数**。两条路径不一致是最坏的失败形态——各自看都正常，只有对比才看得见。

**顺带发现并修掉的一处：`normalize()` 不去中文弯引号。**（原计划立案不修，用户拍板"修，并写好重算脚本"，所以一并做了。）

`_PUNCT_RE` 原来是两个字符串隐式拼接（后半截不是 raw 串），于是 `\\` 塌成一个反斜杠、把后面的 `|` 转义掉了，那两个 ASCII 双引号被当成串边界吃掉。但**真正的问题比手误大得多**：中文弯引号 `“ ” ‘ ’` 从来就没写进去过，`「」『』【】〈〉〔〕` 也没有——数了一下，**37 个常见中文标点没被覆盖**。

实测同一篇稿子换个引号样式，Jaccard 掉到 **0.333**，正好在 0.35 硬闸线之下，开头指纹也对不上。这是一条能绕过查重的路子，而 `【】` 在小红书文案里遍地都是。

逐个往字符类里补是补不完的，所以换成**按 Unicode 类别判**：去 `Cc/Cf · Zs/Zl/Zp · 全部 P* · Sm/Sc/Sk`，**保留 `So`（emoji）**——"换个 emoji 算不算同一篇"是产品问题，不该由一个规范化函数顺手决定。验过没有回归（老口径去掉的，新口径一个不漏），代价是慢 8 倍但绝对值只有 0.32ms / 3000 字。

**改 `normalize` = 让存量指纹作废**，所以配了 `deskcore.cli recompute-fingerprints`。它能修到什么程度不是想修多少就修多少，而是**由指纹表存了什么决定的**——那张表只存 `title` 和 `opening` 前 25 字，**不存正文**：

- `opening_hash` **每一行都能修**（它本来就是 `sha16(normalize(opening))`，而 `opening` 原样存着）。这一路是"单独就判死"的最强信号，能全修回来是关键；
- `ngram_hashes` 只有 `version_id` 非空的行能修。WorkBuddy 经 `commit_drafts` 写进来的行正文已经没了——**修不回来**，返回值里的 `ngram_unrecoverable` 就是那个数，不为 0 会带 warning。

这件事本身值得记：**一个"纯函数"的口径，一旦它的输出被持久化，就不再是纯函数的问题了。** 改它要连着数据迁移一起想，而能不能迁移取决于当初存了什么——这次运气好，最强的那一路存了原料。

**教训**（与 §0.4 是同一件事的第三面）：**先量再改**。"bottom-k 用错了"这个诊断是对的，但它只解释了一部分现象，而我把它当成了全部，还把它写进了 `xfail` 的 reason——下一个人照着做，会发现改完测试还是红的。
